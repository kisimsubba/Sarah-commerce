...
function staatic_platform_check_php_notice() {
    return staatic_platform_check_notice(
        sprintf(
            ...
            __('Staatic for WordPress requires at least PHP version %1$s; detected PHP version %2$s.', 'staatic'),
...
function staatic_platform_check_wordpress_notice() {
    ...
    return staatic_platform_check_notice(
        sprintf(
            /* translators: 1: Minimum version, 2: Installed version. */
            __('Staatic for WordPress requires at least WordPress version %1$s; detected WordPress version %2$s.', 'staatic'),
...
function staatic_platform_check_key_notice() {
    return staatic_platform_check_notice(
        sprintf(
            ...
            __('Staatic for WordPress requires either STAATIC_KEY or AUTH_KEY to be defined within wp-config.php. In order to fix this error, please add the following line to your wp-config.php file and reactivate the plugin: %1$s', 'staatic'),
            sprintf('define(\'STAATIC_KEY\', \'%s\');', wp_generate_password(64, true, true))
        )
    );
}
