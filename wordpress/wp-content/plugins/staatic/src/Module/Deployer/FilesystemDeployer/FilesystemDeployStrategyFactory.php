...
final class FilesystemDeployStrategyFactory
{
    ...
    {
        $targetDirectory = \trailingslashit(\get_option('staatic_filesystem_target_directory'));
        $stagingDirectory = \trailingslashit(\get_option('staatic_work_directory')) . 'staging/';
        $retainPaths = RetainPathsSetting::resolvedValue(
            \get_option('staatic_filesystem_retain_paths') ?: null,
            $targetDirectory
        );
        ...
        $options = [
            ...
            'excludePaths' => [
                \get_option('staatic_work_directory'),
                \get_option('staatic_filesystem_target_directory')
            ],
            'copyOnWindows' => \true
        ];
