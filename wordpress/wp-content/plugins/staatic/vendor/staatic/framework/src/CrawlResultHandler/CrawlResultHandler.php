...
final class CrawlResultHandler implements CrawlResultHandlerInterface
{
    ...
    private function createResult(string $buildId, string $resultId, CrawlResult $crawlResult, Resource $resource) : Result
    {
        ...
        return new Result($resultId, $buildId, $crawlResult->transformedUrl(), $response->getStatusCode(), $resource->md5(), $resource->sha1(), $resource->size(), $mimeType, $charset, $redirectUrl, $crawlResult->url(), $crawlResult->foundOnUrl());
    }
}
