...
final class AmpClientState extends ClientState
{
    ...
    private $logger;
    /**
     * @param callable|null $clientConfigurator
     * @param LoggerInterface|null $logger
     */
    public function __construct($clientConfigurator, int $maxHostConnections, int $maxPendingPushes, &$logger)
    {
        ...
        $this->clientConfigurator = $clientConfigurator instanceof Closure ? $clientConfigurator : function () use ($callable) {
    ...
    public function request($options, $request, $cancellation, &$info, $onProgress, &$handle) : Promise
    {
        ...
        list($client, $connector) = $this->getClient($options);
