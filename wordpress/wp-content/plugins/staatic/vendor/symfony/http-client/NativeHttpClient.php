...
final class NativeHttpClient implements HttpClientInterface, LoggerAwareInterface, ResetInterface
{
    ...
    public function request($method, $url, $options = []) : ResponseInterface
    {
        ...
        if ($onProgress = $options['on_progress']) {
            ...
            $onProgress = static function () use(&$info, $maxDuration) {
                if ($info['total_time'] >= $maxDuration) {
                    throw new TransportException(\sprintf('Max duration was reached for "%s".', \implode('', $info['url'])));
                }
            };
        }
        $notification = static function (int $code, int $severity, $msg, int $msgCode, int $dlNow, int $dlSize) use($onProgress, &$info) {
        ...
        $resolver = static function ($multi) use($context, $options, $url, &$info, $onProgress) {
            list($host, $port) = self::parseHostPort($url, $info);
     ...
     */
    private static function dnsResolve(string $host, NativeClientState $multi, array &$info, $onProgress) : string
    {
     ...
     * @param Closure|null $onProgress
     */
    private static function createRedirectResolver(array $options, string $host, $proxy, array &$info, $onProgress) : Closure
    {
        $redirectHeaders = [];
        ...
        return static function (NativeClientState $multi, $location, $context) use(&$redirectHeaders, $proxy, &$info, $maxRedirects, $onProgress) {
     ...
     * @param mixed[]|null $proxy
     */
    private static function configureHeadersAndProxy($context, string $host, array $requestHeaders, $proxy, bool $isSsl) : bool
    {
        if (null === $proxy) {
