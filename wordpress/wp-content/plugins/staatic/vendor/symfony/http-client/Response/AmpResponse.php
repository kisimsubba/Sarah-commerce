...
final class AmpResponse implements ResponseInterface, StreamableInterface
{
     ...
     */
    public function __construct(AmpClientState $multi, Request $request, array $options, $logger)
    {
        $this->multi = $multi;
     ...
     * @return void
     */
    private static function schedule(self $response, array &$runningResponses)
    {
     ...
     * @return void
     */
    private static function perform(ClientState $multi, array &$responses = null)
    {
        if ($responses) {
     ...
     * @param LoggerInterface|null $logger
     */
    private static function generateResponse(Request $request, AmpClientState $multi, string $id, array &$info, array &$headers, CancellationTokenSource $canceller, array &$options, Closure $onProgress, &$handle, $logger, Promise &$pause)
    {
     ...
     * @param LoggerInterface|null $logger
     */
    private static function followRedirects(Request $originRequest, AmpClientState $multi, array &$info, array &$headers, CancellationTokenSource $canceller, array $options, Closure $onProgress, &$handle, $logger, Promise &$pause)
    {
        (yield $pause);
        ...
        while (\true) {
            ...
            foreach ($originRequest->getRawHeaders() as list($name, $value)) {
     ...
     * @return void
     */
    private static function addResponseHeaders(Response $response, array &$info, array &$headers)
    {
        ...
        foreach ($response->getRawHeaders() as list($name, $value)) {
     ...
     * @param LoggerInterface|null $logger
     */
    private static function getPushedResponse(Request $request, AmpClientState $multi, array &$info, array &$headers, array $options, $logger)
    {
        if ('' !== $options['body']) {
            return null;
        }
        $authority = $request->getUri()->getAuthority();
        foreach ($multi->pushedResponses[$authority] ?? [] as $i => list($pushedUrl, $pushDeferred, $pushedRequest, $pushedResponse, $parentOptions)) {
     ...
     * @return void
     */
    private static function stopLoop()
    {
        if (null !== self::$delay) {
