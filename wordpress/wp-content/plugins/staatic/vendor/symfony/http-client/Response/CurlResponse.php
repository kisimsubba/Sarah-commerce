...
final class CurlResponse implements ResponseInterface, StreamableInterface
{
     ...
     * @return void
     */
    private static function schedule($response, &$runningResponses)
    {
     ...
     * @return void
     */
    private static function perform($multi, &$responses = null)
    {
        if (self::$performing) {
            if ($responses) {
     ...
     * @param callable|null $resolveRedirect
     * @param LoggerInterface|null $logger
     */
    private static function parseHeaderLine($ch, string $data, array &$info, array &$headers, $options, CurlClientState $multi, int $id, &$location, $resolveRedirect, $logger) : int
    {
        $waitFor = @\curl_getinfo($ch, \CURLINFO_PRIVATE) ?: '_0';
        if ('H' !== $waitFor[0]) {
            return \strlen($data);
