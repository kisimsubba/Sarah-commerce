...
final class NativeResponse implements ResponseInterface, StreamableInterface
{
     ...
     */
    public function __construct(NativeClientState $multi, $context, string $url, array $options, array &$info, callable $resolver, $onProgress, $logger)
    {
        $this->multi = $multi;
    ...
    private function open()
    {
        ...
        try {
            ...
            list($resolver, $url) = ($this->resolver)($this->multi);
            while (\true) {
    ...
    {
        foreach ($multi->openHandles as $i => list($pauseExpiry, $h, $buffer, $onProgress)) {
            if ($pauseExpiry) {
    ...
    private static function select(ClientState $multi, float $timeout) : int
    {
        ...
        $_ = $handles = [];
        $now = null;
        foreach ($multi->openHandles as list($pauseExpiry, $h)) {
            if (null === $h) {
                continue;
            }
